# 👁️ doc-parser: VLM-Powered Document Parser for Advanced RAG
...
**doc-parser**는 시각-언어 모델(VLM)을 활용해 문서의 **시각적 레이아웃과 논리적 계층을 이해하고 구조화**하는 문서 파싱 도구입니다. 
...
[opendataloader-bench](https://github.com/opendataloader/opendataloader-bench) 기준 200여 개의 문서를 대상으로 평가한 결과, **표 구조 인식(TEDS)** 및 **헤딩 계층 구조화(MHS)** 부문에서 **가장 높은 성능**을 달성했습니다.
...
| 파서 (Parser) | NID | TEDS | MHS | Overall | 비고 |
|------|-----|------|-----|---------|------|
| opendataloader-hybrid | 0.9355 | 0.9276 | 0.8057 | 0.9034 | |
| **doc-parser (Ours)** | **0.9212** | **0.9358** | **0.8219** | **0.8993** | 👑 **표(Table), 헤딩(Heading) 1위** |
| docling | 0.8995 | 0.8871 | 0.8019 | 0.8766 | |
| marker | 0.8897 | 0.8076 | 0.7956 | 0.8608 | |
| opendataloader | 0.9127 | 0.4942 | 0.7404 | 0.8393 | |
| mineru | 0.8574 | 0.8730 | 0.7430 | 0.8311 | |
| markitdown | 0.8786 | 0.0000 | 0.0000 | 0.5832 | |

### 2. 기여도 분석 (Ablation Study)

doc-parser의 두 핵심 아키텍처인 **'멀티모달 텍스트 레이어 주입'**과 **'프롬프트 엔지니어링'**이 성능에 미치는 영향을 정량화했습니다.
...
doc-parser는 문서를 단순한 텍스트의 나열이 아닌, 검색에 최적화된 **계층적 메타데이터(Breadcrumbs)**로 변환합니다.
...
### 1. 설치 (Installation)
```bash
git clone https://github.com/jeongheart/doc-parser.git
cd doc-parser
...
[Together AI](https://www.together.ai/)에서 API 키를 발급받아 환경 변수로 설정합니다.
```bash
